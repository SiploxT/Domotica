...
const char* mqtt_server = "192.tecnoteis.1.101"; 
...
WiFiClient espClient15;
PubSubClient client(espClient15);
bool ledAceso = false;
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "casa/actuadores/led15") {
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ESP32Client")) {
      ...
      client.subscribe("casa/actuadores/led15");
    } else {
...
void loop() {
  ...
  if (botonEstado == HIGH) { // Se presionamos o botón
    if(!ledAceso) {
      ...
      client.publish("casa/actuadores/led15","LED aceso");
      client.publish("casa/sensores/boton15","Botón premido");
    }
    delay(5000);
  } else{
    if (ledAceso){
      ...
      client.publish("casa/actuadores/led15","LED apagado");
    }
  }
}
